@php
    $seoDesc =
        ...
        $carta->titolo .
        ' della collezione ' .
        ($carta->collezione->nome ?? '') .
        '. Artista: ' .
        ($carta->artista->nominativo ?? '') .
        '.';
    $seoImg = $carta->immagine_asset;
    $seoUrl = route('carte.show', $carta->id_carta);
@endphp
<x-app-layout :title="$carta->titolo" :description="$seoDesc" :ogImage="$seoImg" :ogUrl="$seoUrl" :canonical="$seoUrl">
                    ...
                    <div class="d-flex flex-wrap gap-2 mt-3">
                        ...
                        @auth
                            @if (Auth::user()->isAdmin())
                                <a href="{{ route('admin.carte.edit', $carta->id_carta) }}" class="btn btn-warning">&#9998;
                                    Modifica</a>
                                <a href="{{ route('admin.collezioni.show', $carta->col_id_collezione) }}"
                                    class="btn btn-outline-warning">Vai alla Collezione</a>
                            @endif
                        @endauth
                    </div>
                </div>
