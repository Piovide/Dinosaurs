@php
    $seoDesc = 'Tutte le carte illustrate da ' . $artista->nominativo . ' su ' . config('app.name') . '.';
    $seoUrl = route('artisti.show', $artista->id_artista);
@endphp
<x-app-layout :title="$artista->nominativo" :description="$seoDesc" :canonical="$seoUrl">
                ...
                <div>
                    ...
                    @if ($artista->data_nascita)
                        <p class="text-muted mb-1">
                            <x-icon name="calendar3" size="sm" />
                            {{ \Carbon\Carbon::parse($artista->data_nascita)->translatedFormat('d F Y') }}
                        </p>
                    @endif
