        ...
        <div class="modal-content">
            ...
            <form id="loginForm" method="POST" action="{{ route('auth.login') }}">
                ...
                <div class="modal-footer">
                    <p class="me-auto text-sm">Non hai un account? <a href="#"
                            onclick="switchToRegister()">Registrati qui</a></p>
        ...
        <div class="modal-content">
            ...
            <form id="registerForm" method="POST" action="{{ route('auth.register') }}">
                ...
                <div class="modal-body">
                    ...
                    @endif
                    <div class="mb-3">
                        <label for="register-username" class="form-label">Username <span
                    ...
                    <div class="mb-3">
                        <label for="register-password-confirm" class="form-label">Conferma Password <span
                                class="text-danger">*</span></label>
                ...
                <div class="modal-footer">
                    <p class="me-auto text-sm">Hai già un account? <a href="#" onclick="switchToLogin()">Accedi
                            qui</a></p>
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annulla</button>
...
<script>
    ...
    // Auto-open modal if server returned validation errors (only on non-auth pages)
    @php $isAuthPage = request()->routeIs('auth.login', 'auth.register'); @endphp
    @if (session('open_modal') === 'login' && !$isAuthPage)
        document.addEventListener('DOMContentLoaded', function() {
            openLoginModal();
        });
    @elseif (session('open_modal') === 'register' && !$isAuthPage)
        document.addEventListener('DOMContentLoaded', function() {
            openRegisterModal();
        });
    @endif
